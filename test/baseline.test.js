...
async function writeBasicProject(root, buttonText = 'Save') {
  ...
  await writeFile(path.join(root, 'src/locales/en.json'), JSON.stringify({
    home: { title: 'Welcome back' },
    common: { ok: 'OK' },
  }));
  await writeFile(path.join(root, 'src/locales/zh.json'), JSON.stringify({
    home: { title: 'Welcome back' },
    common: { ok: 'OK' },
  }));
  await writeFile(path.join(root, 'src/components/UserPanel.vue'), `<template>
  <section>
...
`);
  await writeFile(path.join(root, 'i18n-smell.config.mjs'), `
    export default {
...
test('stale baseline issues disappear when baseline is updated', async () => {
  ...
  try {
    ...
    assert.equal(result.status, 0);
    const parsed = JSON.parse(await readFile(baseline, 'utf8'));
    assert.equal(parsed.issues.some((issue) => issue.id === 'identical:zh:removed.key'), false);
    assert.equal(parsed.issues.some((issue) => issue.id === 'identical:zh:home.title'), true);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test('baseline update writes only current non-ignored findings', async () => {
  ...
  try {
    ...
    assert.equal(result.status, 0);
    const parsed = JSON.parse(await readFile(baseline, 'utf8'));
    assert.equal(parsed.issues.some((issue) => issue.value === 'OK'), false);
