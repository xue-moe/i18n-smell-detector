...
test('CLI debug mode prints stack traces for troubleshooting', () => {
  const normal = run(['check', '--config', 'missing.config.mjs']);
  const debug = run(['check', '--config', 'missing.config.mjs', '--debug']);

  assert.equal(normal.status, 2);
  assert.doesNotMatch(normal.stderr, /AppError|at resolveConfigPath/);
  assert.equal(debug.status, 2);
  assert.match(debug.stderr, /code: CONFIG_NOT_FOUND/);
  assert.match(debug.stderr, /at resolveConfigPath/);
});
