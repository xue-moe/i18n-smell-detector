...
import { checkPlaceholders, extractPlaceholders } from '../src/check-placeholders.js';
...
test('extractPlaceholders supports common placeholder styles', async () => {
  ...
  try {
    assert.deepEqual(
      extractPlaceholders('Hello {name} {{ count }} %s %d %(item)s $1', config.placeholderPatterns),
      ['$1', '%(item)s', '%d', '%s', '{name}', '{{ count }}']
    );
  } finally {
