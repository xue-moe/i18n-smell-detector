...
test('checkHardcodedStrings detects Vue template text and attributes', async () => {
  ...
  assert.ok(visible.some((issue) => issue.file === 'basic.vue' && issue.value === 'Profile settings' && issue.severity === 'high'));
  assert.ok(visible.some((issue) => issue.file === 'basic.vue' && issue.value === 'Save' && issue.severity === 'medium'));
  assert.ok(visible.some((issue) => issue.file === 'attributes.vue' && issue.kind === 'vue-attribute:placeholder' && issue.value === 'Search'));
  assert.ok(visible.some((issue) => issue.file === 'attributes.vue' && issue.kind === 'vue-attribute:title' && issue.value === 'Search users'));
});
...
test('checkHardcodedStrings reports malformed Vue files clearly', async () => {
  await assert.rejects(
    () => checkHardcodedStrings({ ...config, source: ['broken.vue'] }, fixtureDir),
    /Malformed Vue/
  );
});
